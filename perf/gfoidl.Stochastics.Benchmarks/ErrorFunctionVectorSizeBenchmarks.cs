...
namespace gfoidl.Stochastics.Benchmarks
{
    ...
    public class ErrorFunctionVectorSizeBenchmarks : IBenchmark
    {
        public void Run()
        {
            const int align = -25;
            var benchs = new ErrorFunctionVectorSizeBenchmarks();
            Console.WriteLine($"{nameof(benchs.NetBase),align}: {benchs.NetBase()}");
            Console.WriteLine($"{nameof(benchs.ErfCpp_Vector_8),align}: {benchs.ErfCpp_Vector_8()}");
            Console.WriteLine($"{nameof(benchs.ErfCpp_Vector_10),align}: {benchs.ErfCpp_Vector_10()}");
            Console.WriteLine($"{nameof(benchs.ErfCpp_Vector_16),align}: {benchs.ErfCpp_Vector_16()}");
            Console.WriteLine($"{nameof(benchs.ErfCpp_Vector_32),align}: {benchs.ErfCpp_Vector_32()}");
            Console.WriteLine($"{nameof(benchs.ErfCpp_Vector_64),align}: {benchs.ErfCpp_Vector_64()}");
#if !DEBUG
            BenchmarkRunner.Run<ErrorFunctionVectorSizeBenchmarks>();
#endif
        }
        //---------------------------------------------------------------------
        private double[] _array;
        //---------------------------------------------------------------------
        public ErrorFunctionVectorSizeBenchmarks()
        {
#if DEBUG
            const int n = 20;
#else
        ...
        //---------------------------------------------------------------------
        [Benchmark]
        public unsafe double ErfCpp_Vector_8() => ErfCpp_Vector(8);
        //---------------------------------------------------------------------
        [Benchmark]
        public unsafe double ErfCpp_Vector_10() => ErfCpp_Vector(10);
        //---------------------------------------------------------------------
        [Benchmark]
        public unsafe double ErfCpp_Vector_16() => ErfCpp_Vector(16);
        //---------------------------------------------------------------------
        [Benchmark]
        public unsafe double ErfCpp_Vector_32() => ErfCpp_Vector(32);
        //---------------------------------------------------------------------
        [Benchmark]
        public unsafe double ErfCpp_Vector_64() => ErfCpp_Vector(64);
        //---------------------------------------------------------------------
...
#region Constants
        ...
        //---------------------------------------------------------------------
        private unsafe double ErfCpp_Vector(int size)
        {
            double e = 0;
            int n    = _array.Length;

            fixed (double* pArray = _array)
            {
                double* arr = pArray;
                int i       = 0;
                ...
                for (; i < n - size; i += size)
                {
                    e += ErfCpp_Vector(arr, size);
                    arr += size;
                }

                e += ErfCpp_Vector(arr, n - i);
            }

            return e;
        }
        //---------------------------------------------------------------------
        private static unsafe double ErfCpp_Vector(double* arr, int size)
        {
            double* values  = stackalloc double[size];
            double* results = stackalloc double[size];

            for (int i = 0; i < size; ++i)
                values[i] = arr[i];

            gaussian_error_function_vector(values, results, size);
